...
def read_file(default=[],filename=config["inv"]):
    """
...
def write_file(data, filename=config["inv"]):
    """
    ...
    try:
        with open(filename, "w+") as file:
            clean = json.dumps(data)  # adding inventory to file
            file.write(clean)
            return True
    except OSError as e:
        print(f"{e}")
...
def add_items():
    ...
    inventory[serial] = {'name': name, 'quantity': quantity, 'ppu': price}  # adding inventory to the dictionary
    write_file(inventory)
...
def send_mail(data):
    ...
    for item in data["list"]:
        email_invoice_list += f"""{ca_spaces(item["time"],36)}{ca_spaces(item["name"])}{ca_spaces(item["serial"], 8)}{ca_spaces(item["quantity"])}{ca_spaces(item["ppu"], price=True)}\n    """
    email_text = f"""<pre>\n
    {"-" * 110}
    {ca_spaces("Vedavyas", 110)}
    {ca_spaces("N01583267", 110)}
    {("-" * 110)}
...
def sales_invoice():
    ...
    inventory = read_file()
    invoice = read_file(filename=config["sal"])
    ...
    if send_mail(write_invoice) is False:
        ...
        return False
    # Write to the file
    write_file(write_invoice, config["sal"])
...
def display_inventory():
    ...
    if inventory:
        ...
        for serial, item in inventory.items():
            ...
            print("%30s%30s%30s%30s\n" % (item['name'], serial, item['quantity'], "$" + str(round(item['ppu'], 2))))
    else:
